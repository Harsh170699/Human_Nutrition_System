...
(function (Drupal, $) {
  Drupal.behaviors.wviAction = {
    attach: function attach(context) {
      ...
      if ($(window).width() > 959) {
        actionContainer.each(function (index) {
          position_indicator($(this).find('.component-action--title.active'));
          setTimeout(function () {
      ...
      function position_indicator(e) {
        thisActionContainer = e.closest(actionContainer);
        setTimeout(function () {
          var top = e.position().top + e.height() / 2 - 3;
...
(function (Drupal, $) {
  Drupal.behaviors.wviExposedFilters = {
    attach: function attach(context) {
      ...
      var filters = filtersWrapper.find('.exposed-filters--container');
      var filterLegend = filtersWrapper.find('> legend');

      function position_filters(e) {
        var filterOffset = filtersWrapper.offset().top;
        var footerOffset = $('footer').last().offset().top;
        var filtersTop = filtersWrapper.height() + 1;
        var filtersHeight = footerOffset - (filterOffset + filtersTop);
        filters.css('top', filtersTop);
        filters.css('height', filtersHeight);
      }

      if (!filters.attr('data')) {
        // Add a data attribute to the exposed filters to track if reloaded.
        filters.attr('data', 'loaded');

        filterLegend.on('click', function (e) {
        ...
        $('.exposed-filters--container .fieldgroup legend').on('click', function (e) {
        ...
        $('.exposed-filters--container #edit-groups-field-data-label--wrapper .checkboxes-group').on('click', function (e) {
        ...
        position_filters();
        $('.exposed-filters--container .checkbox-group-container').addClass('open');
      } else if (context != document) {
        // Reposition filters after an AJAX request.
        position_filters();
      } else {
        // Only apply window handlers if the context is the document.
      ...
      // After pressing browser back button, resolve discrepancies with checkboxes.
      $(context).ready(function () {
        $('.views-exposed-form input:not(#edit-sort-bef-combine-created-desc)').each(function () {
  ...
  Drupal.behaviors.wviExposedSort = {
    attach: function attach(context) {
      ...
      sortWrapper.find('legend').on('click', function (e) {
        if (sortWrapper.hasClass('active')) {
          sortWrapper.removeClass('active');
          setTimeout(function () {
            sortWrapper.find('.fieldset-wrapper').hide();
          }, 500);
        } else {
          sortWrapper.find('.fieldset-wrapper').show();
          setTimeout(function () {
            sortWrapper.addClass('active');
          }, 200);
        }
      });
    }
...
(function (Drupal) {
  Drupal.behaviors.wviAlert = {
    attach: function attach(context) {
      ...
      try {
        for (var _iterator = alerts[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          alert.addEventListener('click', this.dismissAlert);
        }
      } catch (err) {
    ...
    dismissAlert: function dismissAlert(e) {
...
(function (Drupal) {
  Drupal.behaviors.wviTopNav = {
    attach: function attach(context) {
      ...
      if (menuGroup) {
        var offsetHeight = document.querySelector('.region-topnav').offsetHeight;

        // Set initial value.
        offsetHeight = offsetHeight + "px";
        menuGroup.style.top = offsetHeight;

        // Change the value as the window resizes and the top nav changes with it.
        window.addEventListener('resize', function (e) {
          var newOffsetHeight = document.querySelector('.region-topnav').offsetHeight;
          newOffsetHeight = newOffsetHeight + "px";
          menuGroup.style.top = newOffsetHeight;
        });
      }
      ...
      var handleDesktop = function handleDesktop() {
        var dropDown = document.querySelector('.region-topnav');
        ...
        var classes = {
          ...
          // We handle the expansion of the top nav and the actual dropdown separately.
        };var ddMenuWrapToggle = function ddMenuWrapToggle(e) {
          // The target is the <a> tag in the header.
          var target = e.target;
          var acceptableTags = ['A', 'DIV', 'UL', 'LI'];
          if (!dropDown.classList.contains(classes.open) && acceptableTags.includes(target.tagName)) {
            dropDown.classList.toggle(classes.open);
          }
          // If the target is not in our list of tags we are no longer hovering on the menu/it can be closed.
          else if (dropDown.classList.contains(classes.open) && !acceptableTags.includes(target.tagName)) {
              dropDown.classList.toggle(classes.open);
            }
        ...
        dropDownTrigger.addEventListener('mouseover', ddMenuWrapToggle);

        dropDownTrigger.addEventListener('mouseleave', ddMenuWrapToggle);

        var dropDownMenuItem = dropDown.querySelectorAll('.menu-item--expanded');

        dropDownMenuItem.forEach(function (el, i) {
          var ddMenuWrap = el.querySelector('.menu_link_content');
          var ddMenu = el.querySelector('.menu-level-1');

          var ddMenuToggle = function ddMenuToggle() {
            ddMenuWrap.classList.toggle(classes.active);
            ddMenu.classList.toggle(classes.active);
          };

          el.addEventListener('mouseover', function (e) {
            // "El" is a link so we need to prevent default for mobile devices.
            if ('ontouchstart' in document.documentElement) {
              e.preventDefault;
              el.classList.toggle(classes.hover);
            }

            if (!dropDown.classList.contains(classes.open) && !el.classList.contains(classes.hover)) {
              // Delay only the first opening of the dropdown. If we do this with CSS transiton-delay
              // the interstitial menu fades are delayed, so we do the first one with JS only.
              setTimeout(function () {
                ddMenuToggle();
              }, 400);
            } else {
              // Otherwise we're going between li and can just fade them.
              ddMenuToggle();
            }
          });
...
(function (Drupal) {
  ...
  Drupal.behaviors.wviPowerclaim = {
    ...
    resizeText: function resizeText(powerclaims) {
      textFit(powerclaims, { alignHoriz: true, alignVert: true, maxFontSize: 800 });
    }
  };
  ...
  Drupal.behaviors.wviGroupMenu = {
    attach: function attach(context) {
      ...
      if (menuGroup) {
        var _menu = menuGroup.getElementsByTagName('ul')[0];
      } else {
        return;
      }
      ...
      function toggleDownloadLink() {
        ...
        var _loop = function _loop(teaser) {
          teaser.addEventListener('mouseover', function (e) {
            teaser.getElementsByClassName('publication__download-link')[0].style.display = 'inline-block';
          });
          teaser.addEventListener('mouseout', function (e) {
            teaser.getElementsByClassName('publication__download-link')[0].style.display = 'none';
          });
        };
        ...
        try {
          for (var _iterator2 = teasers[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
...
(function (Drupal, $) {
  Drupal.behaviors.wviSlider = {
    attach: function attach() {
      ...
      if ($(".js-slider.container--hero .container__slider").length) {
        $(".js-slider.container--hero .container__slider").slick({
          infinite: true,
          slidesToShow: 1,
          slidesToScroll: 1,
          dots: false
        });
        video_sliders.push(".js-slider.container--hero .container__slider");
      }

      if ($(".js-slider.container--gallery-item .container__slider").length) {
        $(".js-slider.container--gallery-item .container__slider").slick({
          infinite: true,
          slidesToShow: 1,
          slidesToScroll: 1,
          dots: false
      ...
      if ($(".js-slider .component-media-wrapper").length) {
        $(".js-slider .component-media-wrapper").slick({
          infinite: true,
          slidesToShow: 1,
          slidesToScroll: 1,
          dots: false
        });
        video_sliders.push(".js-slider .component-media-wrapper");
      }

      if ($(".js-slider.field--name-field-gallery .gallery__slider").length) {
        $(".js-slider.field--name-field-gallery .gallery__slider").slick({
          infinite: true,
          slidesToShow: 1,
          slidesToScroll: 1,
          dots: false
      ...
      if ($(".js-slider.field--name-field-event-image-gallery .gallery__slider").length) {
        $(".js-slider.field--name-field-event-image-gallery .gallery__slider").slick({
          infinite: true,
          slidesToShow: 1,
          slidesToScroll: 1,
          dots: false
      ...
      if ($('.container--icon.js-slider .container__slider').length) {
        $('.container--icon.js-slider .container__slider').slick({
          infinite: true,
          dots: false,
          mobileFirst: true,
          responsive: [{
      ...
      if ($('.container--counter-with-media.js-slider .container__slider').length) {
        $('.container--counter-with-media.js-slider .container__slider').slick({
          infinite: false,
          dots: false,
          mobileFirst: true,
          responsive: [{
      ...
      if ($('.container--claim.js-slider .container__slider').length) {
        $(".container--claim.js-slider .container__slider").slick({
          infinite: true,
          dots: false,
          mobileFirst: true,
          responsive: [{
      ...
      if ($('.container--card.js-slider .container__slider').length) {
        $(".container--card.js-slider .container__slider").slick({
          infinite: true,
          dots: false,
          mobileFirst: true,
          responsive: [{
      ...
      if ($('.container--callout.js-slider .container__slider').length) {
        $(".container--callout.js-slider .container__slider").slick({
          infinite: true,
          dots: false,
          mobileFirst: true,
          responsive: [{
     ...
     */
    initTimelineSlider: function initTimelineSlider() {
      var containerSlider = $('.container--timeline.js-slider .container__slider');
    ...
    initStatSlider: function initStatSlider() {
      var containerSlider = $('.container--stat.js-slider .container__slider');
      containerSlider.slick({
        infinite: true,
        mobileFirst: true,
        responsive: [{
  ...
  Drupal.behaviors.wviSliderBioProfiles = {
    attach: function attach(context, settings) {
      ...
      $('.views-view--bio-profiles', context).slick({
        infinite: true,
        mobileFirst: true,
        responsive: [{
          breakpoint: 1024,
      ...
      $(window).on('resize orientationchange', function () {
        if ($('.views-view--bio-profiles').length) {
          $('.views-view--bio-profiles').slick('resize');
        }
      });
    }
  };
...
(function (Drupal, $) {
  Drupal.behaviors.wviCounter = {
    ...
    attach: function attach(context) {
      ...
      var counterTop = document.querySelector(".counter__title");
      var countFrom = document.querySelector(".active-number").innerHTML;
      countFrom = parseInt(countFrom.replace(/,/g, ''));
      var countTo = document.querySelector(".field--name-field-counter-value").innerHTML;
      ...
      // If the page loads with the counter in view, start counting!
      if (isInViewPort(counterTop)) {
        animateCount(counter, duration, easingType);
        return;
      }

      document.addEventListener("scroll", animate);

      // Once the user scrolls, we check to see if she's gone far enough.
      function animate() {
        if (isInViewPort(counterTop)) {
          animateCount(counter, duration, easingType);
        }
      ...
      // Check if the user can see the counter.
      function isInViewPort(ourElement) {
        var bounding = ourElement.getBoundingClientRect();
        return bounding.top >= 0 && bounding.left >= 0 && bounding.bottom <= (window.innerHeight || document.documentElement.clientHeight) && bounding.right <= (window.innerWidth || document.documentElement.clientWidth);
      }

      // Handles changing colors of numbers during counting.
      function colorChange(entireNumber) {
        var currNumber = $('.active-number').html();
        var pattern = /[^0|^,]/g;
        ...
        $('.active-number').html(secondHalf);
        $('.zeros').html(firstHalf);
      }

      function animateCount(counter, duration) {
        $({ countNum: countFrom }).animate({ countNum: countTo }, {
          duration: duration,
          easing: easingType,
          step: function step() {
            $('.active-number').html(Math.floor(this.countNum).toString().padStart(padding, '0').replace(/\B(?=(\d{3})+(?!\d))/g, ",") // Add commas in number.
            );
            colorChange($('.entire-number').html());
          },
          complete: function complete() {
            $('.active-number').html(this.countNum.toString().padStart(padding, '0').replace(/\B(?=(\d{3})+(?!\d))/g, ",") // Add commas in number.
            );
            colorChange($('.entire-number').html());
          }
        });
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.fixedTopNav = {
    attach: function attach(context, settings) {
      ...
      var menuGroupSticky = $('.menu--group-sticky');
      var localTasks = $('.block--wvi-2018-primary-local-tasks');
      var currentScroll = $(window).scrollTop();

      if (topMenuContainer.length > 0 && alertContainer.length > 0) {
        setTimeout(calcStickyMenuTop, 150);
        // Wait until finish resizing to execute.
        $(window).scroll(function () {
          currentScroll = $(window).scrollTop();
          clearTimeout(window.scrollFinished);
          // On finish scroll, recalculate the current active menu item.
          window.scrollFinished = setTimeout(calcStickyMenuTop, 100);
        });

        $(alertContainer).find('.wvi-alert--toggle').change(function () {
          setTimeout(calcStickyMenuTop, 75);
        });
      }

      function calcStickyMenuTop() {
        // Calculate how much we have scrolled pass the alert container and set it as the main menu spacing to top.
        var headerHeight = alertContainer.height() + parseInt($('body').css('paddingTop')) - currentScroll;
        headerHeight = headerHeight >= 0 ? headerHeight : 0;
        $(topMenuContainer).css('top', headerHeight + 'px');
        if (headerHeight === 0) {
        ...
        // Calculate how much we have scrolled pass the menu group container and set it as fixed behind the main menu.
        if (menuGroupSticky.length > 0) {
          var viewportWidth = window.innerWidth || document.documentElement.clientWidth;
          $(menuGroupSticky).css('top', '0px');
          var menuGroupWrapper = $(menuGroupSticky).find('.menu--group__wrapper');
          var menuGroupOffset = menuGroupSticky.offset().top - parseInt($('body').css('paddingTop')) - currentScroll;
          if (viewportWidth >= 960) {
            if (menuGroupOffset <= 0) {
              $(menuGroupWrapper).addClass('is-fixed').css('top', $(topMenuContainer).height() + headerHeight + 'px');
            } else {
              if (headerHeight + parseInt($(topMenuContainer).height()) >= menuGroupOffset) {
                $(menuGroupSticky).css('height', $(menuGroupSticky).height() + 'px');
                if (localTasks.length > 0) {
                  $(menuGroupWrapper).addClass('is-fixed').css('top', $(topMenuContainer).height() + headerHeight + localTasks.height() + 'px');
                } else {
                  $(menuGroupWrapper).addClass('is-fixed').css('top', $(topMenuContainer).height() + headerHeight + 'px');
                }
              } else {
                $(menuGroupSticky).css('height', 'auto');
                $(menuGroupWrapper).css('top', '0px').removeClass('is-fixed');
              }
            }
          }
        }
